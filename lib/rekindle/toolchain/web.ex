defmodule Rekindle.Toolchain.Web do
  ...
  @doc false
  @spec render_bootstrap(String.t(), String.t(), [String.t()]) :: binary()
  def render_bootstrap(javascript, wasm, hot_styles)
      when is_binary(javascript) and is_binary(wasm) and is_list(hot_styles) do
    @bootstrap_template
    |> String.replace("__REKINDLE_ENTRY__", CanonicalValue.encode!("./" <> javascript))
    |> String.replace("__REKINDLE_WASM__", CanonicalValue.encode!("./" <> wasm))
    |> String.replace("__REKINDLE_HOT_STYLES__", CanonicalValue.encode!(hot_styles))
  end
