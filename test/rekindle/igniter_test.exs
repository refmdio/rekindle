defmodule Rekindle.IgniterTest do
  ...
  test "atomically upgrades the supported prior client and preserves application-owned files" do
    prior = prior_client()

    installed =
      prior
      |> Map.put("src/app.rs", "// application-owned UI\n")
      |> Map.put("public/.gitkeep", "application-owned placeholder\n")
      |> put_client_files(project())
      |> Igniter.create_new_file("client/public/theme.css", "application-owned asset\n")
      |> apply_igniter!()
      |> RekindleIgniter.install(
        client_path: "client",
        targets: [:web, :desktop],
    ...
    for relative <- owned_paths(current) do
      assert virtual_file(installed, Path.join("client", relative)) == current[relative]
    end

    assert virtual_file(installed, "client/src/app.rs") == "// application-owned UI\n"

    assert virtual_file(installed, "client/public/.gitkeep") ==
             "application-owned placeholder\n"

    assert virtual_file(installed, "client/public/theme.css") == "application-owned asset\n"

    marker = Jason.decode!(virtual_file(installed, "client/.rekindle-client.json"))
    ...
    for relative <- Map.keys(current) ++ ["public/theme.css"] do
      path = Path.join("client", relative)
      assert virtual_file(reinstalled, path) == virtual_file(installed, path)
    end
  end

  test "prior upgrade accepts recorded and known template digests" do
    prior = prior_client()
    recorded_contents = "# recognized prior Cargo template variant\n"

    prior =
      prior
      |> Map.put("Cargo.toml", recorded_contents)
      |> rewrite_prior_digest("Cargo.toml", sha256(recorded_contents))
      |> rewrite_prior_digest("rust-toolchain.toml", String.duplicate("0", 64))
    ...
    current = current_client()
    assert virtual_file(upgraded, "client/Cargo.toml") == current["Cargo.toml"]

    assert virtual_file(upgraded, "client/rust-toolchain.toml") ==
             current["rust-toolchain.toml"]
  end

  test "rejects unsupported or malformed prior identities without changing client bytes" do
    prior = prior_client()
    marker = Jason.decode!(prior[".rekindle-client.json"])

    invalid_markers = [
      marker |> Map.put("template_version", "0") |> reseal_prior_marker(),
      Map.delete(marker, "package"),
      Map.put(marker, "extra", true),
      marker |> Map.put("application_id", "foreign") |> reseal_prior_marker(),
      marker
      |> update_in(["owned_files"], &tl/1)
      |> reseal_prior_marker(),
      marker
      |> update_in(["owned_files"], &Enum.reverse/1)
      |> reseal_prior_marker(),
      marker
      |> update_in(["owned_files"], fn entries ->
        entries ++ [%{"path" => "foreign", "template_sha256" => String.duplicate("a", 64)}]
      end)
      |> reseal_prior_marker()
    ]

    for invalid_marker <- invalid_markers do
      candidate =
        Map.put(
          prior,
      ...
      initial = candidate |> put_client_files(project()) |> apply_igniter!()
      ...
      assert Enum.any?(rejected.issues, &String.contains?(to_string(&1), "client marker"))

      for {relative, contents} <- candidate do
        assert virtual_file(rejected, Path.join("client", relative)) == contents
      end
    end
  end

  test "rejects missing or modified prior owned files before applying any upgrade" do
    prior = prior_client()

    for candidate <- [
          Map.delete(prior, "src/bin/web.rs"),
          Map.put(prior, "src/bin/web.rs", "fn main() {}\n")
        ] do
      initial = candidate |> put_client_files(project()) |> apply_igniter!()
      ...
      unless Map.has_key?(candidate, "src/bin/web.rs") do
        assert {:error, _reason} = Rewrite.source(rejected.rewrite, "client/src/bin/web.rs")
        refute Map.has_key?(rejected.assigns.test_files, "client/src/bin/web.rs")
      end
    end
  end
  ...
  defp put_client_files(files, igniter) do
    Enum.reduce(files, igniter, fn {relative, contents}, acc ->
      Igniter.create_new_file(acc, Path.join("client", relative), contents)
    end)
  end

  defp owned_paths(files) do
    files[".rekindle-client.json"]
    |> Jason.decode!()
    |> Map.fetch!("owned_files")
    |> Enum.map(& &1["path"])
  end

  defp rewrite_prior_digest(files, path, digest) do
    marker =
      files[".rekindle-client.json"]
      |> Jason.decode!()
      |> update_in(["owned_files"], fn entries ->
        Enum.map(entries, fn
          %{"path" => ^path} = entry -> Map.put(entry, "template_sha256", digest)
          entry -> entry
        end)
      end)
      |> reseal_prior_marker()

    Map.put(files, ".rekindle-client.json", Rekindle.CanonicalValue.encode!(marker) <> "\n")
  end

  defp reseal_prior_marker(marker) do
    entries = marker["owned_files"]

    if is_list(entries) and
         Enum.any?(entries, &match?(%{"path" => ".rekindle-client.json"}, &1)) do
      base =
        Map.put(
          marker,
          "owned_files",
      ...
      digest = sha256(Rekindle.CanonicalValue.encode!(base) <> "\n")

      Map.update!(marker, "owned_files", fn values ->
        Enum.map(values, fn
          %{"path" => ".rekindle-client.json"} = entry ->
            Map.put(entry, "template_sha256", digest)
